...
前面两个实验攒的东西在这里汇合了。FCOB 灯带已经能跟 ESP32 正常通信，WiFi 网页服务器也知道怎么搭了。下一步很自然;做一个像样的取色器。要能拖拽的真色环，灯带颜色实时跟着变，不是几个 RGB 滑块凑合一下。
...
坑在通道顺序：`NeoGrbwFeature` 映射的是（红、白、绿、蓝），跟你从名字上猜的完全不是一回事。我在这上面浪费了整整 20 分钟，最后老老实实回去翻了库的文档才搞明白。
...
这种项目属于硬件没几样、但成品看起来特别像回事的类型;就一块 ESP32 加一条灯带，但网页界面一打开，感觉就像个正经产品。色环拖起来很解压，场景动画演示效果也好，白光提取让粉彩和暖色调看起来是那个味道而不是灰蒙蒙的。后面还想接着搞;多加几个场景，接 MQTT，看看能不能挂到 Home Assistant 上。不过现阶段它已经做到了我最开始想要的效果：选个颜色，灯带亮起来，不用装任何 App。
