...
GPIO 是 MCU 的通用输入输出接口，可以配成输入（检测外部电平）或输出（产生高/低电平）。但它的本质是数字控制接口;高电平大约 3.3V（或 5V），低电平约 0V，对应逻辑 1 和 0。它能输出的电流非常有限，通常 20mA 左右。
...
PWM 是在全开和全关之间高速切换，通过改变导通时间的占比（占空比）来调节平均功率。LED 因为视觉暂留，看到的亮度是连续的（只要频率够高）。
