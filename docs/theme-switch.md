...
## 追加（2026-09-20）：副本存活时长必须等于落位时长，黑洞出场与星空错开

**用户报的两个现象**（首页，light→dark）：
1. 「圆环覆盖画面后，这时候理应动画结束，但这时候又闪一下，然后才允许点击按钮」
2. 「黑洞会卡住一下子，然后画面突然出现星空，然后黑洞才开始播放」
   → 期望：「先让星空从暗到亮过渡出现，然后是黑洞同样的从暗到亮过渡出现并且播放」

**根因只有一个：`COPY_FADE_MS` 比落位动画短。** 它原本是写死的 **700ms**，而 `sfArrive` 是 1600ms、`ARRIVE_MS` 是 1700ms。副本在动画只走到约 77% 时就被撤走，剩下 23% 是在**已经露出的页面**上爬完的——那一下就是「闪」，那个尾巴就是「黑洞才开始播放」。

逐帧实测（1720×1000，修复前）：

| t | 事件 | lens opacity |
| --- | --- | --- |
| 6214 | `#lens-canvas` 的 `display` 翻成 `block` | 读到 **0.90**（还没跑动画！） |
| 6240 | `.sf-arrive` 生效 | 归 0 重新往上爬 |
| 6987 | **副本移除** | 只有 **0.77**，还差 0.13 |
| 7637 | 才到 0.90 | 页面已经露出 **650ms** |

**修法**

- `const COPY_FADE_MS = ARRIVE_MS;` —— 副本一直盖到落位动画真正跑完才撤。⚠️ **不要再把它改短**：任何小于 `ARRIVE_MS` 的值都会让渐变尾巴露在页面上。
- ⚠️ 顺带修掉一个**会直接让主题切换挂掉**的隐患：`ARRIVE_MS` 原本声明在文件下方（原 284 行），而 `COPY_FADE_MS` 是模块级初始化 `= ARRIVE_MS`。**模块级初始化按源码顺序求值**，引用下方的 `const` 会抛 TDZ `ReferenceError`，整个模块加载失败。所以 `ARRIVE_MS` 必须留在文件顶部的时长常量区（`GUARD_MS` 旁边）。
- **黑洞出场改为与星空错开**（`BH_FADE_DELAY_MS = 765`，约落位动画的 45%）：`arriveBackground` 里先摘掉黑洞层的 `is-ready` 重新武装那 0.5s 淡入，延迟 765ms 再加回来，同时确保视频在播。
  - ⚠️ 播放不能等淡入：`display: none` 可能让 `loop` + `muted` 的视频停在暂停态。先 `play()`，再让它自己淡入。
  - ⚠️ 层在浅色主题下是 `display: none`，所以落地那一帧它会以 `opacity: 1`（`.is-ready` 的值）回来**一帧没跑动画**。这一帧仍被副本盖着，所以看不见——**但这就是不能把副本时长改短的原因**。

修复后实测：

| t | 事件 |
| --- | --- |
| 1581 | `#lens-canvas` display 翻 block（仍被副本盖住） |
| 1615 | 黑洞层失去 `is-ready` → 重新武装淡入 |
| 2394 | 黑洞层重新获得 `is-ready` → 开始淡入（比落位晚 779ms） |
| 2894 | 黑洞层 opacity 到 1.00 |
| ≈8480 | 透镜 opacity 到 0.90 |
| ≈8540 | **副本移除** —— 透镜 0.90、黑洞 1.00、视频 `paused=0` |

→ 移除瞬间两者都已完全落位，不再有任何可见的渐变尾巴。

⚠️ **首页没有「星空」层可淡入。** `html[data-lens="on"] #starfield-canvas { display: none }`（§10）让引力透镜**取代**星空，首页渲染的是 `#lens-canvas`。所以「先星空后黑洞」在首页实际表现为「先场（透镜）后黑洞」。若要让两者同时可见，那是**视觉设计变更**，需要先和用户确认。
...
⚠️ headless 里 `Page.captureScreenshot` 单帧要 0.5–0.9s，**拍不到一轮波**（当时 620ms，2026-09-19 起 900ms，见文末最后一节）；上面所有结论都来自「同一任务里同步读 DOM」+「把拷贝删掉后拍活页面」这两种能绕开时序的测法。
...
## 追加（2026-09-19）：圆环放慢 —— 问题在缓动曲线，不在时长（v1.47）

用户报：「点击 light/dark mode 按钮时…覆盖的速度太快了，肉眼看就像好几帧里面快速的进行了一次覆盖，不够丝滑。优化一下，放慢他覆盖的速度，或者补帧？」

**先回答「补帧」：没有帧可补。** 圆是 Web Animations API 的 `clip-path` 补间，浏览器每帧按**墙钟时间**算半径 —— 它本来就是帧率无关的，60fps 下就有 60 个不同位置。看到「好几帧」不是帧不够，是**曲线把运动全挤进了开头**。

**真因：`cubic-bezier(0.22, 1, 0.36, 1)` 是 ease-out-quint**，`REVEAL_MS` 620 只是个上限 —— **40% 的半径落在最初 10% 的时间里**，90% 落在 37%，剩下的时间边缘几乎不动。解析解（`docs/pitfalls.md` §6 的测法复核过，见下）：

| 时长的百分比 | 10% | 25% | 50% | 75% | 100% |
| --- | --- | --- | --- | --- | --- |
| 旧半径 | .401 | .674 | **.961** | .994 | 1.000 |
| 旧**面积**（r²） | .161 | .454 | **.923** | .988 | 1.000 |

⚠️ **判据要用面积，不是半径。** 圆铺到屏幕上的像素 ∝ r²，眼睛读的是「画面被盖掉多少」：旧曲线走到**一半时间**时半径已经 .961，转换过来是**92% 的屏幕已经盖完**；走完头三分之一就盖掉了约 75%。所以一次 620ms 的切换，实际可感的扫描只有 ~230ms，后面 47% 的时钟只把边缘推了 3% 的半径 —— 这正是用户描述的「好几帧就扫完了」。

**改法：换曲线 + 给曲线留出时间。** `EASING` 改成 `cubic-bezier(0.25, 0.1, 0.25, 1)`（CSS 的 `ease`，**故意写开**，这样文档里的百分比可以直接对着它核），`REVEAL_MS` 620 → **900**，`RIPPLE_MS` 460 → **660**（⚠️ 保持「叠上来的圆比第一个快」这条 volley 约定，比例 620/460 原样保留为 900/660；⚠️ `GUARD_MS` 1600 必须留在两者之上）。

新曲线：半径的 25/50/75/90% 落在时长的 **18/29/45/62%** —— 没有死尾，边缘一直在走。

**实测**（`/projects/`、1700×900、真实点击、页面内 8ms 轮询 `getComputedStyle(el).clipPath`）：

| | 旧（620ms / easeOutQuint） | 新（900ms / ease） |
| --- | --- | --- |
| 半径到 50% | 213ms | **400ms** |
| 半径到 90% | 486ms | **690ms** |
| 半径到 97% | 486ms | **804ms** |
| 最后一个可见半径 | 782ms | **971ms** |

⚠️ 表里是**从点击算起**的绝对时间，含 headless 里 `anim.pause()` 那一帧等待（本轮实测 ~130ms；真机是一帧 ~16ms）。扣掉它之后归一化：50% 落在 900ms 的 **30%**、90% 落在 **62%** —— 与解析解预测的 29.3% / 62.3% 逐位吻合 ✓ 曲线确实按 `ease` 在跑。

rAF 采样里变化最直观：**旧**曲线 446ms 就到 .983，之后一直平着（死尾）；**新**曲线一路在爬 —— `.577 → .785 → .839 → .882 → .917 → .944 → .971 → .985 → .995 → 1.000`，每一帧都在动。

⚠️ **帧率这条**：headless 判不了。它是软件渲染，而且 `/projects/` 本身 idle 就只有 33–50ms/帧（书架 + canvas）。本轮揭示期间 rAF 帧间隔 p50 **50.1ms** / p90 **66.8ms** —— 基本就是该页基线，只有开头一个 **233ms** 的尖峰（那是拷贝的首次光栅，与决策 23「已知代价」记的 50–120ms 同源）。也就是说**没有证据表明揭示本身是瓶颈**；如果真机上仍然觉得顿，下一个嫌疑是 `clip-path` 有没有走合成器，而不是曲线。
