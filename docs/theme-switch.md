...
## 追加（2026-09-20）：星空 / 黑洞落位必须「同帧换肤 + 同步装 class」，且他俩错开出场

**用户报的现象**（首页，light→dark）：「圆环覆盖完画面后，要等好几秒才闪，闪完后才开始播放动效」，并提示「之前好像没有这个问题」。期望：「先透镜，然后黑洞，并且他俩是丝滑渐入」。

**用户的直觉是对的。** `git show ac804e5:src/scripts/theme-transition.ts` 里的 `landBurst` 是：

```js
const final = pending;
dropLayers();                     // 副本先撕
pending = null;
if (final) applyInstant(final);   // 同一帧再换主题
```

**当时根本没有 `COPY_FADE_MS`。** 它是后来新加的，并从 700ms 一度改到 `ARRIVE_MS`（1700ms）——那 1700ms 就是「等好几秒」：副本原地不动 1.7 秒，期间的换肤与副本撕走叠成一次硬切。

**修法（回到原设计，并修掉原设计里那一下「突然出现」）**

- `landBurst` 恢复为「同一帧先撕副本、再落主题」。`COPY_FADE_MS`、`coverOff()`、`coverTimer`/`dropTimer`、`landingTail` 全部删除——`burstLocked()` 现在就是 `burstInFlight()`，没有任何 tail。
- ⚠️ **`arriveBackground()` 必须同步装 class，不能包 `requestAnimationFrame`。** 它在 `applyInstant` 里被调用，也就是画布刚脱离 `display: none` 的那一帧；推迟一帧会留下一帧画在画布**静止透明度**（0.85 / 0.9）上的画面——这正是「闪一下，然后才开始播放」的来源。
- 黑洞与透镜**错开**出场：先摘掉 `is-ready` 让它自己的 0.5s 淡出，765ms（`BH_FADE_DELAY_MS`）后再加回来淡入，并在那时确保视频在播。
- 新增 `arrivalTimer`：连点产生的两轮 arrival 之间，前一轮的清理定时器不能把后一轮的 class 提前摘掉。

**实测**（1720×1000，light→dark）：

| t | 事件 |
| --- | --- |
| 1562 | 副本移除 —— 透镜 opacity 已是 **0.00**，正在往上走 |
| 1603→2987 | 透镜 **0.00 → 0.90** 平滑 |
| 1603→2704 | 黑洞 **0.86 → 1.00** 平滑（先降后升，即错开） |
| 3271 | arrival class 摘除，数值保持 |

→ 不再有「闪」也不再有等待；两者都是渐入，且透镜在前、黑洞在后。

⚠️ **不要再给落地加任何「保持副本」的等待。** 副本的作用是遮住换肤那一帧，而落位动画是给人**看**的——页面必须在它开始时就已经露出。
