...
| 改项目/实验卡片（含毛玻璃）  | `src/components/ProjectCard.astro`（`.card` 的 `backdrop-filter: blur(20px)`；⚠️ `main`/`.card` 入场动画必须用 `animation-fill-mode: backwards`，残留 transform 会让 Chrome 丢掉模糊）                                                                                      |
