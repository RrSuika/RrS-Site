...
| 改引力透镜 / 调每个圈的位置    | `src/components/GravitationalLens.astro`（分层/速度/透镜数学）+ `src/styles/global.css` §10 `#lens-canvas` 的 `--lens-*` 变量；现场调参：导航栏齿轮图标 / 反引号键 / `?lens=tune`（方向键调每个圈半径/吸积盘/中心，Shift ×10 / Alt ×0.1，COPY 按钮复制参数）；关掉透镜=去掉三个首页的 `blackhole` prop |
